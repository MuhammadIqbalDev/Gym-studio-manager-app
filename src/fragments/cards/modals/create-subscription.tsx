...
const CreateSubscription: React.FC<CreateSubscriptionProps> = ({
  ...
  const onSubmit = async (data: SubscriptionType) => {
    ...
    const body = {
      ...
      subscription_limit_frame: data.subscription_limit || false,
      subscription_limit_times: data.subscription_limit || false,
      subscription_iteration_type: Number.parseInt(data.subscription_duration)
    }
    try {
      await mongoSelector({
        from: ENDPOINTS.EVENT.CREATE_SUBSCRIPTION_POST,
        method: 'POST',
        body
      })
    } catch (err) {
  ...
  const renderSubscriptionBody = () => (
    <>
      ...
      <div className={styles.subscriptionLimit}>
        <LabelWrapper
          style={{ gridArea: 'limit' }}
          error={'subscription_limit' in errors ? 'Required' : null}>
          {() => (
            <Controller
              control={control}
  ...
  const renderSubscriptionFooter = () => (
    <div className={styles.subscriptionFooter}>
      <button disabled={isSubmitting} type="button" style={{ gridArea: 'button' }} onClick={handleSubmit(onSubmit)}>
        סיום
      </button>
      <LabelWrapper
        title="מחיר"
