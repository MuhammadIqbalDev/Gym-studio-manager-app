...
const useCards: CollectionFeatureHook<CardType, { studio_id: string | undefined }> = ({
  studio_id
}) => {
  swrKey = {
    method: 'POST',
    from: ENDPOINTS.EVENT.GET_ALL_CARDS_POST,
    body: { studio_id }
  }
  const { data, error, isLoading, isValidating } = useSWR<{ data: CardType[] }, ApiError>(
    swrKey,
...
export const getCardsMutateKey = () => swrKey
